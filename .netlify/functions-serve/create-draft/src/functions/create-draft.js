...
// functions/create-draft.js
exports.handler = async (ev) => {
  const { v4: uuidv4 } = require_dist();
  ...
  const trimmedPlayerNames = playerNames.split(",").map((x2) => x2.trim());
  console.log(2);
  const postResponse = await fetch("https://api.buttercms.com/v2/pages/", {
    method: "post",
    body: JSON.stringify({
      title: draftName,
      slug: draftId,
  ...
  return {
    ...
    body: JSON.stringify({
      ...
      draftName,
      draftOrder: JSON.stringify([...Array(parseFloat(rounds)).keys()].flatMap((i2) => (i2 + 1) % 2 ? trimmedPlayerNames : trimmedPlayerNames.slice().reverse()))
    }, null, "	")
  };
};
